using UnityEngine;
using System.Collections;

// キャラクターを移動させる。
// Chapter3
public class wolfMove : MonoBehaviour
{
    // 重力値.
    //const float GravityPower = 0f;
    //　目的地についたとみなす停止距離.
    const float StoppingDistance = 0.1f;

    // 現在の移動速度.
    Vector3 velocity = Vector3.zero;
    // キャラクターコントローラーのキャッシュ.
    CharacterController characterController;
    // 到着したか（到着した true/到着していない false)
    public bool arrived = false;

    // 向きを強制的に指示するか.
    bool forceRotate = false;

    // 強制的に向かせたい方向.
    Vector3 forceRotateDirection;

    // 目的地.
    public Vector3 destination;

    // 移動速度.
    public float walkSpeed = 6.0f;

    // 回転速度.
    public float rotationSpeed = 360.0f;

    private Transform player;

    private Transform wolf;



    // Use this for initialization
    void Start()
    {
        StartCoroutine("search");
        characterController = GetComponent<CharacterController>();
        destination = transform.position;
        wolf = this.gameObject.transform;
    }

    // Update is called once per frame
    void Update()
    {
        // 移動速度velocityを更新する
        if (characterController.isGrounded)
        {
            
        }
        else
        {
            //　水平面での移動を考えるのでXZのみ扱う.
            Vector3 destinationX = destination;
            destinationX.z = transform.position.z;// 高さを目的地と現在地を同じにしておく.
            destinationX.y = transform.position.y;

            //********* ここからXZのみで考える. ********
            // 目的地までの距離と方角を求める.
            Vector2 direction = (destinationX - transform.position).normalized;
            float distance = Vector3.Distance(transform.position, destinationX);

            // 現在の速度を退避.
            Vector3 currentVelocity = velocity;

            //　目的地にちかづいたら到着..
            if (arrived || distance < StoppingDistance)
                arrived = true;


            // 移動速度を求める.
            if (arrived)
                velocity = Vector3.zero;
            else
                velocity = direction * walkSpeed;


            // スムーズに補間.
            velocity = Vector3.Lerp(currentVelocity, velocity, Mathf.Min(Time.deltaTime * 5.0f, 1.0f));
            //velocity.y = 0;


            if (!forceRotate)
            {
                // 向きを行きたい方向に向ける.
                if (velocity.magnitude > 0.1f && !arrived)
                { // 移動してなかったら向きは更新しない.
                    Quaternion characterTargetRotation = Quaternion.LookRotation(direction);
                    transform.rotation = Quaternion.RotateTowards(transform.rotation, characterTargetRotation, rotationSpeed * Time.deltaTime);
                }
            }

        }

        // 接地していたら思いっきり地面に押し付ける.
        // (UnityのCharactorControllerの特性のため）
        Vector3 snapGround = Vector3.zero;
        if (characterController.isGrounded)
            snapGround = Vector3.down;

        // CharacterControllerを使って動かす.
        characterController.Move(velocity * Time.deltaTime + snapGround);

        if (characterController.velocity.magnitude < 0.1f)
            arrived = true;

        // 強制的に向きを変えるを解除.
        if (forceRotate && Vector3.Dot(transform.forward, forceRotateDirection) > 0.99f)
            forceRotate = false;


    }

    
    IEnumerator search()
    {
        //0.2秒後に、プレイヤー(ターゲット)を設定
        yield return new WaitForSeconds(0.2f);
        player = GameObject.FindGameObjectWithTag("Player").transform;

    }
    
    
    // 目的地を設定する.引数destinationは目的地.
    public void SetDestination1(Vector3 destination)
    {
        arrived = false;
        this.destination = destination;
        if (velocity.x > 0)
        {
            transform.localScale = new Vector3(-1, 1, 1);
        }
        else
        {
            transform.localScale = new Vector3(1, 1, 1);
        }
    }

    public void SetDestination2(Vector3 destination)
    {
        arrived = false;
        Vector3 playerPos = player.position;
        Vector3 wolfPos = wolf.position;
        this.destination = playerPos;
        if (playerPos.x > wolfPos.x)
        {
            transform.localScale = new Vector3(1, 1, 1);
        }
        if(playerPos.x < wolfPos.x)
        {
            transform.localScale = new Vector3(-1, 1, 1);
        }
    }

    // 目的地に到着したかを調べる. true　到着した/ false 到着していない.
    public bool Arrived()
    {
        return arrived;
    }


}